...
<html>
    <head>
        ...
        <title>Editar | Receptores de Correo</title>
        <script type="text/javascript" language="JavaScript" >
    ...
    <body>
        <div class="container">
            <div class="row">
                ...
                <div class="col-md-9">
                    <form method="post" id="form_CREARfambotanica" action="recursos/acciones.php?tarea=8&id=<?php echo $_GET["id"]; ?>">
                    ...
                    <?php
                        $con=Conexion();
                        $sql_Email="select * from emails where idemails='".$_GET["id"]."'";
			$result_Email=mysql_query($sql_Email,$con) or die(mysql_error()); 
                        if(mysql_num_rows($result_Email)>0){
                            $email = mysql_fetch_assoc($result_Email);                                                            
                        }                    
                    ?>                         
                            ...
                            <?php
                                ...
                                if(mysql_num_rows($result_listaEMPRESA)>0){
                                    while ($fila = mysql_fetch_assoc($result_listaEMPRESA)) {
                                        if($email["idempresa"]==$fila["idempresa"]){
                    ...
                    <div class="col-md-12 contiene_entrada">
                        ...
                        <div class="col-md-12"><input type="text" class="form-control" value="<?php echo $email["nombre"] ?>"  id="nombre" name="nombre" maxlength="60" required="required" /></div>
                    </div>
                    <div class="col-md-12 contiene_entrada">
                        <div class="col-md-12 titulo_entrada">Correo Electronico</div>
                        <div class="col-md-12"><input type="text" class="form-control" value="<?php echo $email["correo"] ?>"  id="correo" name="correo" maxlength="60" required="required" /></div>
                    </div>                        
                    <div class="col-md-12 contiene_entrada">
