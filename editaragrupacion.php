    ...
    <body>
        <div class="container">
            <div class="row">
                ...
                <div class="col-md-9">
                    <form method="post" id="form_CREARfambotanica" action="recursos/acciones.php?tarea=19&id=<?php echo $_GET["id"]; ?>">
                        
                        <div class="col-md-12 titulopagina" style="margin-top: 125px;">Agrupación de Cuentas</div>
                        <div class="col-md-12">
                            <div class="btn-group" role="group" aria-label="...">
                            <button type="button" onclick=redirigir("insertagrupacion.php?idempresa=<?php echo $agrupacion["idempresa"]; ?>") class="btn btn-default boton">Crear Nuevo Elemento +</button>
                            <button type="button" onclick=redirigir("listaragrupacion.php?id=<?php echo $agrupacion["idempresa"]; ?>")  class="btn btn-default boton">Listar Elementos</button>
                            </div>
                        </div>
                        <div class="col-md-12 subtitulopagina">
                            ...
                            <?php
                                ...
                                $sqlcuentasasociadas="select * from agrupacioncuentas where idagrupacion='".$_GET["id"]."'";
                            ...
                            <?php
                                ...
                                if(mysql_num_rows($result_listaAGRUPACION)>0){
                                    while ($fila = mysql_fetch_assoc($result_listaAGRUPACION)) {
                                        if($agrupacion["idtipoagrupacion"]==$fila["idtipoagrupacion"]){
                                            echo "<option selected='selected' value='".$fila["idtipoagrupacion"]."'>".$fila["nombre"]."</option>";
                                        }else{
                                ...
                                <?php 
                                    ...
                                    if(mysql_num_rows($result_listaCuenta)>0){
                                        while ($cuenta = mysql_fetch_assoc($result_listaCuenta)) {
                                            $sqlcuentasasociadas="select * from agrupacioncuentas where idagrupacion='".$_GET["id"]."' and codigocuenta='".$cuenta["codigo"]."'";
                                            $resultcuentasasociadas=mysql_query($sqlcuentasasociadas,$con) or die(mysql_error());
                                            if(mysql_num_rows($resultcuentasasociadas)==0){
